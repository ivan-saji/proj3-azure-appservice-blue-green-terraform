#Setup Autoscaling for the webapp service plan
...
resource "azurerm_monitor_autoscale_setting" "autoscale_setting" {
  ...
  target_resource_id  = azurerm_service_plan.asp_appservice_lab.id
  ...
  profile {
    ...
    rule {
      metric_trigger {
        ...
        metric_resource_id = azurerm_service_plan.asp_appservice_lab.id
        time_grain         = "PT1M"
        statistic          = "Average"
        time_window        = "PT5M"
